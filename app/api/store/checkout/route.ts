...
type AllowedSku = (typeof SKU)[keyof typeof SKU];

const SKU_PRICE_ATOMIC: Record<AllowedSku, number> = {
...
export async function POST(req: Request) {
  ...
  if (!SKU_ALLOWLIST.has(sku as AllowedSku)) return new Response('Invalid SKU', { status: 400 });
  const allowedSku = sku as AllowedSku;
  const idempotency_key = randomId();
  const amount = SKU_PRICE_ATOMIC[allowedSku];
  ...
  const payload = buildX402Payload(purchase.id, allowedSku, amount);
